...
export async function POST(request) {
  try {
    ...
    // Gather context
    const { rows: recentMoods } = await sql`
    ...
    if (!GROQ_API_KEY) {
      return await generateFallbackTasks(recentMoods, userId);
    }
    ...
    if (parsed.tasks && Array.isArray(parsed.tasks)) {
      for (const task of parsed.tasks.slice(0, 3)) {
        ...
        await sql`
          ...
          VALUES (${id}, ${userId}, ${task.title}, ${task.description}, ${category}, 'ai')
        `;
...
async function generateFallbackTasks(recentMoods, userId) {
  ...
  const taskSets = {
    ...
    triste: [
      ...
      { title: 'Mensagem para alguém querido', description: 'Envie uma mensagem simples para alguém que você gosta. Conexão social é um antídoto poderoso para a tristeza.', category: 'social' },
    ],
    feliz: [
  ...
  for (const task of selectedTasks) {
    ...
    await sql`
      INSERT INTO tasks (id, user_id, title, description, category, generated_by) 
      VALUES (${id}, ${userId}, ${task.title}, ${task.description}, ${task.category}, 'ai')
    `;
    newTasks.push({ id, ...task, is_completed: 0, generated_by: 'ai' });
  }
